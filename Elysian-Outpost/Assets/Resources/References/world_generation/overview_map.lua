...
function OverviewMap:__init(biome, landscaper)
   self._biome = biome
   self._landscaper = landscaper

   self._tile_size = biome:get_tile_size()
   self._macro_block_size = biome:get_macro_block_size()
   self._feature_size = biome:get_feature_block_size()
...
function OverviewMap:_get_terrain_code(i, j, elevation_map, feature_map)
   ...
   -- since macro_blocks are 2x the feature size, all 4 cells have the same value
   -- if this changes, sort and return the 3rd item (one of the tied medians)
   local elevation = elevation_map:get(i, j)
   return self._biome:get_terrain_code(elevation)
end
...
function _overview_map_to_feature_map_index(i)
   ...
   -- *0.5 to scale
   -- +1 to base 1
   -- +1 to align to macro_block boundary (recall that tiles are offset/overlap by half macro_block)
