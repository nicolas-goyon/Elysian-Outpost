...
function HeightMapRenderer:__init(biome)
   self._biome = biome
   ...
   local mountains_info = terrain_info.mountains
   local rock_layers = {}
   local terrain_tags = {}
   ...
   for i, tag in ipairs(terrain_tags) do
      rock_layers[i] = {
         terrain_tag = tag,
         max_height = mountains_info.height_base + mountains_info.step_size*i
      }
   end
...
function HeightMapRenderer:render_height_map_to_region(region3, height_map, underground_height_map)
   ...
   region3:optimize('heightmaprenderer:render_height_map_to_region()')
end
