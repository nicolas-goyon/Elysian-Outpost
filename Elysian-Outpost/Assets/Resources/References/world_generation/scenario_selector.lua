...
function ScenarioSelector:__init(biome,rng)
   self._biome = biome
   self._rng = rng
   self._scenarios = {}
end
...
function ScenarioSelector:contains(name)
   return self._scenarios[name] ~= nil
end
...
function ScenarioSelector:_get_scenario_volume(properties)
   local feature_width, feature_length = self._biome:get_feature_dimensions(properties.size.width, properties.size.length)
