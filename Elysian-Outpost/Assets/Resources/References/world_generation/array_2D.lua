...
function Array2D:copy_to_1D_array()
   local dst = {}
   local size = self.width * self.height
   for i=1, size do
      dst[i] = self[i]
...
function Array2D:copy_to_nested_arrays()
   local dst = {}
   local offset, row
...
function Array2D:fill_block(x, y, block_width, block_height, fn)
   local index
